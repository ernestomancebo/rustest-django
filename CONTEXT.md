...
- **Runner option**: a knob pytest-django exposes as a CLI flag (`--reuse-db`, `--ds`, `--no-migrations`). rustest has no flag registration, so a runner option is expressed as an environment variable or a `pyproject.toml` key instead.
...
- **DB blocker**: the guard that raises when a test touches the database without `db`, `transactional_db`, or the `django_db` marker.
